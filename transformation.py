...
cur = conn.cursor()
cur.execute("CREATE SCHEMA IF NOT EXISTS iceberg.report with (LOCATION = 's3a://sample-lakehouse/report/')")
query = """CREATE TABLE IF NOT EXISTS iceberg.report.order_by_service(
...
for i in range(len(df)):
    ...
    if str(completed_at) == "NaT":
        query = f"INSERT INTO iceberg.report.order_by_service VALUES (TIMESTAMP '{accepted_at}',TIMESTAMP '{boarded_at}', TIMESTAMP '{picked_up_at}', NULL, TIMESTAMP'{cancelled_at}', '{servicename}', {price})"
        cur.execute(query)
    if str(cancelled_at) == "NaT":
        query = f"INSERT INTO iceberg.report.order_by_service VALUES (TIMESTAMP '{accepted_at}',TIMESTAMP '{boarded_at}', TIMESTAMP '{picked_up_at}', TIMESTAMP '{completed_at}', NULL, '{servicename}', {price})"
        cur.execute(query)
